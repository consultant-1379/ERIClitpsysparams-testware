...
class Story2327Story5774(GenericTest):
    ...
    def _find_values_sysctl(self, node, option):
        ...
        cmd = "/sbin/sysctl -a | grep '{0}'".format(option)
        stdout, stderr, rc = self.run_command(node, cmd, su_root=True)
        self.assertEquals([], stderr)
    ...
    @attr('pre-reg', 'revert', 'story2327_5774', 'story2327_5774_tc01',
          'cdb_priority1')
    def test_01_p_create_remove_system_param_positive(self):
        ...
        sysctl_key2 = "kernel.msgmnb"
        sysctl_key3 = "kernel.msgmax"
        ...
        try:
            # 1. Find the sysparam-node-config already on node1
            sysparam_node1_config = self.find(
                self.test_ms, "/deployments", "sysparam-node-config")[0]

            # 2.  Check file for the preexisting key(a) on both nodes
            #     and find its value
            ...
            # 3.  Create system-param on node1 with preexisting
            #    key(a) in the file
            ...
            # 4. Find the sysparam-node-config already on node2
            sysparam_node2_config = self.find(
                self.test_ms, "/deployments", "sysparam-node-config")[1]

            # 5.  Check file for the preexisting key(b) on both nodes
            #     and find its value
            ...
            # 6. Create another system-param with key(b) in the file
            #    on node2
            ...
            # 7. Update another pre-existing key(c) in the file manually
            ...
            # 10. Check sysctl.conf file contains updated preexisting key(a)
            ...
            # 11.  Check the value is not updated on node2 config file
            get_key1_val = self._find_keyvalue_in_sysctl_conf(
                test_node2, sysctl_key1)
            self.assertEqual(node2_key1_val, get_key1_val)

            # 12. Check sysctl.conf file contains updated key(b)
            ...
            # 17. Remove the system-param item-types that have been created
            ...
            # 18. Check their states are, "ForRemoval"
            ...
            # 23.Check the keys, (a),(b) and (c) has removed
            ...
            # 24.Check the keys, (a) and (c) is not removed from memory
    ...
    @attr('pre-reg', 'revert', 'story2327_5774', 'story2327_5774_tc02')
    def test_02_n_check_system_parameter_validation_negative(self):
        ...
        # 2. Attempt to create system-param value with an empty string
        rule_sets = []
        rule_set = {
        ...
        for rule in rule_sets:
         ...
         # 13. Attempt to remove non existing "system-param node config"
        # from the model
    ...
    @attr('pre-reg', 'revert', 'story2327_5774', 'story2327_5774_tc03')
    def test_03_p_update_system_parameter_positive(self):
        ...
        # 6. Update sysparam2 which has invalid key(2) to valid updated key(2)
        props = 'key="{0}"'.format(sysctl_updated_key2)
        self._update_system_param_props(
            sysparam_node_config, "sysctltest03b", props)

        # 7. Check the updated sysparam2 state is "initial"
        state_value = self.execute_show_data_cmd(
            self.test_ms, sysparam2, "state")
        self.assertEqual(state_value, "Initial")
        ...
        # 10.  Check the sysparams states are in Applied state.
        state_value = self.execute_show_data_cmd(
            self.test_ms, sysparam1, "state")
        ...
        # 11. Update the value of the sysparm1
        props = ('value="15638"')
        self._update_system_param_props(
            sysparam_node_config, "sysctltest03a", props)

        # 12. Update sysparam3 key(3) with key(4)
        ...
        # 18. Check the value of the sysparam1 is updated in
        # the sysctl.conf file
        updated_key1_val = self._find_keyvalue_in_sysctl_conf(
            test_node1, sysctl_key1)
        self.assertNotEqual(orig_key1_val, updated_key1_val)

        # 19. check syaparam1 value is updated in memory on the target node
        updated_memory_value = self._check_memory_values(test_node1,
                                                         sysctl_key1)
        self.assertEqual(updated_key1_val, updated_memory_value)

        # 20. Check sysparam3 key(3) is removed in sysctl.conf
        self._find_keyvalue_in_sysctl_conf(
            test_node1, sysctl_key3, positive=False)

        # 21. Check sysparam3 key(3) exists in the memory
        self._check_memory_values(test_node1, sysctl_key3)

        # 22. Check sysparam3 key(4) is not added in the config file
        self._find_keyvalue_in_sysctl_conf(
            test_node1, sysctl_key4, positive=False)

    @attr('pre-reg', 'revert', 'story2327_5774', 'story2327_5774_tc04')
    def test_04_p_system_param_export_load_xml(self):
        ...
        try:
            ...
            # 4. Load the sysparam-node-config into model using --merge
            self.execute_cli_load_cmd(
                self.test_ms, n1_config_path, "xml_04a_story2327.xml",
                "--merge")

            # 5. Load the sysparam-node-config into model using --replace
            self.execute_cli_load_cmd(
                self.test_ms, n1_config_path, "xml_04a_story2327.xml",
                "--replace")

            # 6. Export the system-param
            self.execute_cli_export_cmd(
            ...
            # 8. Load the system-param into the model
            self.execute_cli_load_cmd(
            ...
            # 11. Check the created sysparam state is "initial"
            state_value = self.execute_show_data_cmd(
                self.test_ms, system_param, "state")
            self.assertEqual(state_value, "Initial")

            # 12. Load xml file using the --replace
            self.execute_cli_load_cmd(
                self.test_ms, sysparam_node_config,
            ...
            # 15. Check state of items in tree
            self.assertTrue(self.is_all_applied(self.test_ms))

            # 16. Check all parameters in xml file exist in conf file
            self._find_keyvalue_in_sysctl_conf(
                test_node1, sysctl_key1, positive=False)
            ...
            # 17. Load original exported config
            self.execute_cli_load_cmd(
    ...
    @attr('pre-reg', 'revert', 'story2327_5774', 'story2327_5774_tc05')
    def test_05_n_update_invalid_parameter_negative(self):
        ...
        # 2. Create system-param with new key
        ...
        # 5. Check the plan should fail
        self.assertTrue(self.wait_for_plan_state(
            self.test_ms, test_constants.PLAN_FAILED))

        # 6. Check sysctl.conf file doesn't contains key
        self._find_keyvalue_in_sysctl_conf(
            test_node1, sysctl_new_key, positive=False)

        # 7. Check key failed to add parameter
        cmd = self.redhatutils.get_sysctl_cmd(
        ...
        [
         {
          'index': 0,
          'path': None,
          'msg': 'error: "kernel.newkey" is an unknown key',
          }
         ]
        }
        rule_sets.append(rule_set.copy())
        ...
        # 8. Search for the current value of the read only parameter,
        #    net.ipv4.conf.default.mc_forwarding.
        orig_sysctl_key = self._find_values_sysctl(test_node1, sysctl_key)

        # 9. Create system-param with the key,
        #   net.ipv4.conf.default.mc_forwarding and with a value
        #   other than the value currently in the sysctl.conf file
        ...
        # 12. Check the plan should fail
        self.assertTrue(self.wait_for_plan_state(
            self.test_ms, test_constants.PLAN_FAILED))

        # 13. Check sysctl.conf file failed to updated value
        updated_sysctl_key = self._find_values_sysctl(test_node1, sysctl_key)
        self.assertEqual(orig_sysctl_key, updated_sysctl_key)

        # 14. Check puppet failed to update a key in memory
        cmd = self.redhatutils.get_sysctl_cmd(
            '{0}'.format(sysctl_key))
    ...
    @attr('pre-reg', 'revert', 'story2327_5774', 'story2327_5774_tc06')
    def test_06_p_create_remove_system_param_with_slash(self):
        ...
        try:
            ...
            # 2.  Create system-param on node1 with preexisting
            #    key(a) in the file
            props = 'key="{0}" value="{1}"'.format(sysctl_key1, sysctl_value1)
            system_param1 = self._create_system_param(
            ...
            # 5. Check sysctl.conf file contains updated preexisting key(a)
            #   to the value on node1
            key1_val = self._find_keyvalue_in_sysctl_conf(
                test_node1, sysctl_key1)
            self.assertEqual(key1_val, sysctl_key1 + " = " + sysctl_value1)

            # 6. Remove the system-param item-types that have been created
            # and remove manually updated key(c)
            self.execute_cli_remove_cmd(self.test_ms, system_param1)
            ...
            # 9.Check the key has been removed
            # from sysctl.conf file
            self._find_keyvalue_in_sysctl_conf(
                test_node1, sysctl_key1, positive=False)

            # 10.Check the key is not removed from memory
