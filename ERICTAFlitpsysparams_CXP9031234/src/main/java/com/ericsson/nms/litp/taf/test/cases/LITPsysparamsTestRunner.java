...
public class LITPsysparamsTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPsysparamsTestRunner.class);
    ...
    @TestId(id = "CXP9031234-1", title = "Upgrade ERIClitpsysparams rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpsysparamsRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("sysparams");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031234-2", title = "Run python test cases for ERIClitpsysparams package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpsysparamsTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpsysparamsTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031234-3", title = "Parse xml outputs from python tests")
